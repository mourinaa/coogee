...
// 👉 Fetch Invoices
watchEffect(() => {
  console.log(`WATCH INVOICE: ${searchQuery.value}`);
  invoiceListStore
    .fetchInvoices({
      q: searchQuery.value,
    ...
    .then((response) => {
      ...
      console.log(`FETCH EVENTS INVOICE: ${JSON.stringify(data)}`);
    })
    .catch((error) => {
      console.log(error);
    });
...
const resolveInvoiceStatusVariantAndIcon = (status: string) => {
  if (status === 'Partial Payment') return { variant: 'success', icon: 'tabler-circle-half-2' };
  if (status === 'Paid') return { variant: 'warning', icon: 'tabler-chart-pie' };
  if (status === 'Downloaded') return { variant: 'info', icon: 'tabler-arrow-down-circle' };
  if (status === 'Draft') return { variant: 'primary', icon: 'tabler-device-floppy' };
  if (status === 'Sent') return { variant: 'secondary', icon: 'tabler-circle-check' };
    ...
    <VCardText class="d-flex align-center flex-wrap gap-4">
      ...
      <div class="d-flex align-center flex-wrap gap-4">
        ...
        <div class="invoice-list-filter">
          <VTextField v-model="searchQuery" placeholder="Search Invoice" density="compact" />
        </div>
    ...
    <VTable class="text-no-wrap invoice-list-table">
      ...
      <tbody>
        <tr v-for="invoice in invoices" :key="invoice.id" style="height: 3.75rem">
            ...
            <VTooltip>
              <template #activator="{ props }">
                <VAvatar
                  ...
                  variant="tonal"
                >
                  <VIcon :size="24" :icon="resolveInvoiceStatusVariantAndIcon(invoice.invoiceStatus).icon" />
                </VAvatar>
              </template>
          ...
          <!-- 👉 Balance -->
          <td class="text-center">
            <VChip label v-bind="resolveInvoiceBalanceVariant(invoice.balance, invoice.total).chip" size="small">
